...
addEventListener('load', () => {
    ...
    */
    ...
    data = consultaPorGenero(mediaType[Math.floor(Math.random() * 2)], '28');
    ...
    data.then( promise => {
        ...
        while(index < data.length){
            ...
            let imgElement = document.getElementById(`acao-${index+1}`);
...
function consultaPorGenero(mediaType, genreId) {
    ...
    return fetch(`${url}?api_key=${api_key}&language=${language}&sort_by=${sort_by}&include_adult=${include_adult}&include_video=${include_video}&genre_id=${genreId}&page=${page}`, {
        method: "GET",
    }).then(response => { 
        return response.json();
    }).then(data => {
